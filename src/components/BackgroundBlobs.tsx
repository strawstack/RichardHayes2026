...
export function BackgroundBlobs() {
  ...
  return (
    <Canvas
      className="absolute inset-0 w-full h-full bg-amber-50"
      onMount={(canvas: HTMLCanvasElement) => {
        ...
        const blobPattern = (center: Point) => {
          const baseRadius = 250;
          return [
            circleData({
              size: { width, height },
            ...
            circleData({
              ...
              center: sub(center, { x: baseRadius / 4, y: baseRadius / 4 }),
              color: "white",
              drawPath: "stroke",
            }),
          ];
        ...
        circles.current = [
          ...
          slatePattern({ x: 0, y: height - 250 / 2 }),
          blobPattern({ x: width - 250, y: height - 2 * 250 }),
        ].flat();
      }}
      draw={draw}
    ></Canvas>
